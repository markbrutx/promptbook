...
A _composition_ (introduced in [Concepts](/docs/concepts)) has a `name`, a `base` list of fragment ids, and a `rules` list. Each rule has a `when` clause and exactly one action.
