...
The viewer is a "Storybook for prompts": one page that lists every composition, every fragment, every code-prompt, and lets you re-assemble any variant under any context, with the rule cascade explained next to the resolved text. A **Graph** tab flips the same book into an Obsidian-style map of everything connected to everything.
...
- **Canvas (center).** The resolved prompt with each fragment painted a different colour; hover for the fragment id, click to jump into the fragment view. The footer shows the rough token count. Tabs at the top switch between **Canvas** and **Graph**.
...
The **Graph** tab renders the whole book as one force-directed map, the way
Obsidian maps a vault:

- **Nodes** are compositions, fragments, and code-prompts. A fragment's node
  grows with usage, so the shared core of the book is visible at a glance.
- **Edges** exist wherever a composition's base list, one of its rules
  (`add` / `replace` / `forbid` / `order`), or a `${...}` placeholder connects
  two nodes.
- **Click any node** to jump straight to it — a composition opens on the
  canvas, a fragment opens its fragment view.

Use it to answer "what breaks if I edit this fragment?" before touching
anything: every edge out of the node is a prompt that edit reaches.
